...
- [x] 10.1 RED: `tests/cogito/test_artifacts.py` — `artifacts.require(prev)` falla limpiamente (sin crash, sin artefacto parcial) si el artefacto previo falta o está vacío.
- [x] 10.2 GREEN: crear `src/erickfp/cogito/artifacts.py`.
- [x] 10.3 RED: `tests/cogito/test_phases.py` — `duda` detecta ambigüedad (requisitos contradictorios) y pide clarificación sin generar artefacto; entrada clara produce artefacto aceptado.
- [x] 10.4 GREEN: crear `src/erickfp/cogito/phases.py` (duda/divide/ordena/enumera + roles Planner/Coder/Reviewer desde `core/agents`; `save` explícito de session-summary, auto-summary fuera de alcance por YAGNI). **Protocolo de marcadores**: `duda` usa `ACEPTADO:`/`AMBIGUO:` inyectados en las instrucciones de la fase para que el modelo declare explícitamente su decisión (fail-safe: sin marcador reconocido, se trata como ambiguo). Verificado en producción real en el smoke E2E (11.3).
- [x] 10.5 RED: `tests/cogito/test_orchestrator.py` — cadena completa `duda→divide→ordena→enumera` con `MockProvider` + `.ErickFP/` en `tmp_path`; modo interactivo pausa entre fases; modo automático encadena sin pausa. Incluye test adicional de bloqueo real por `adr_traceability` en `ordena`.
- [x] 10.6 GREEN: crear `src/erickfp/cogito/orchestrator.py` (invoca `HookManager` en `PhaseStart`/`PhaseEnd`, agent loop por fase, `artifacts.require`). **Deviación documentada**: `cogito/phases.py` depende de `erickfp.agent.loop.run_turn` (dependencia real no listada en el árbol de Decisión 1 del design, igual que la omisión ya documentada de `agent` en la Fase 8/Lote 4) — permitida sin tocar el contrato `import-linter` porque `cogito` ya está por encima de `agent` en las capas declaradas en `pyproject.toml`.
- [x] 10.7 GREEN: cablear comandos `duda`, `divide`, `ordena`, `enumera` en `cli.py` reutilizando el orquestador. Cubierto por `tests/cli/test_cogito_commands.py` (RED→GREEN, incluye `_slugify`/`_load_role_prompt` y las dos rutas de error: `ArtifactMissingError`/`PhaseBlockedError` → exit_code 1 sin crash).

## Fase 11: Integración final y verificación de robustez

- [x] 11.1 `tests/test_architecture_import_rules.py` + config `import-linter`: valida la regla de dependencia completa (`api→nada`; `provider|tools|memory→api`; `hooks→api`; `cogito→api,provider,tools,hooks,memory`; `cli→todo`). Pasa completo por primera vez tras crear `erickfp.cogito` en la Fase 10 (antes fallaba con "Missing layer erickfp.cogito", condición preexistente desde Lote 1).
- [x] 11.2 Ejecutar `ruff check` y `mypy` sobre `src/erickfp/`; corregir hallazgos. 7 líneas E501 nuevas corregidas (cli.py, cogito/{orchestrator,phases}.py, tests/cogito/*); mypy limpio en 29 archivos.
- [x] 11.3 Smoke manual (E2E, no automatizado): `erickfp init` + `erickfp duda` (Ciclo Cogito) + `erickfp chat` real contra el modelo default del ADR-001 (`gemini/gemma-4-26b-a4b-it`) con API key real del `.env`; documentado en `docs/smoke-e2e.md`. Resultado: `init` OK sin red; 2 intentos de `duda` fallaron con `500 INTERNAL` transitorio real (el retry de 11.5 se activó pero el error persistió en ambos intentos de cada invocación — evidencia real, no simulada); llamada diagnóstica a otro modelo confirmó inestabilidad momentánea del proveedor (503 "high demand"), no un problema de la key/código; tras esperar 15s, `duda` y `chat` completaron exitosamente con contenido real y coherente del modelo. Ninguna llamada imprimió la API key.
- [x] 11.4 Actualizar `openspec/config.yaml` (`verify.test_command: pytest`, `coverage_threshold` acordado: 85%, con margen sobre el 95% real medido con `pytest --cov=erickfp`).
- [x] 11.5 Retry con backoff acotado ante `500 INTERNAL` transitorios en `litellm_gemini.py` (hallazgo del spike 2.2 con Gemma 4: 1 de 11 llamadas reales fallo con 500 esporadico; extender el patron de `scripts/spike_thought_signature.py::_call_with_backoff` a "500"/"INTERNAL"). TDD: `tests/provider/test_litellm_gemini_retry.py` (RED→GREEN, 3 tests: reintenta 1 vez y luego OK, no reintenta errores no transitorios, propaga tras el segundo 500 consecutivo). `sleep_fn` inyectable (default `time.sleep`) para tests rápidos sin esperar el backoff real. **Verificado con evidencia real** en el smoke E2E (11.3): el retry se activó ante un 500 real del proveedor.
- [x] 11.6 Limpiar los 6 `E501` preexistentes de `scripts/spike_thought_signature.py` o excluir `scripts/` del linting (decision menor pendiente desde Lote 2). **Decisión**: se excluyó `scripts/` del linting (`[tool.ruff] extend-exclude = ["scripts"]`) — código de spike exploratorio y desechable, no de producción (YAGNI: no vale la pena mantener limpieza cosmética sobre código de un solo uso).
