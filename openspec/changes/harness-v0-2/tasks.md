...
Convención: cada tarea de implementación es RED (test que falla) → GREEN (mínimo código que lo pasa). STRICT TDD activo. Cada Lote termina con `pytest -q` + `ruff check .` + `mypy src/erickfp` + `lint-imports` en verde antes de avanzar (regla del ciclo 1). 55 escenarios reales de las 11 specs mapeados 1:1 a un test nombrado (conteo verificado en Lote 9, tarea 9.4 — el "56" original de esta línea era una estimación de la fase `sdd-tasks`; ver tabla de trazabilidad completa en `docs/smoke-e2e-v0-2.md`).
...
## Lote 7 — subagentes — [x] COMPLETO (9/9)

- [x] 7.1 RED clase `Agent` extraída; `run_turn` libre queda wrapper con firma preservada (no rompe tests previos). Test: `tests/agent/test_agent_class.py::test_free_run_turn_wraps_agent_class_same_signature`
- [x] 7.2 GREEN `agent/agent.py::Agent` (provider/tools/hooks/policy/compaction/tracker).
- [x] 7.3 RED `Research` se instancia solo con tools read-only; `write_file` no registrada. Test: `tests/subagents/test_research.py::test_research_registry_contains_only_read_only_tools`
- [x] 7.4 GREEN `subagents/research.py`: `Agent` con registry subset + `AllowList` read-only.
- [x] 7.5 RED **TRANSVERSAL (d)** Research NO puede escribir — tool desconocida, sin ejecución. Test: `tests/subagents/test_research.py::test_research_cannot_write_unknown_tool_no_execution`
- [x] 7.6 RED core_guard sigue activo dentro del subagente. Test: `tests/subagents/test_research.py::test_core_guard_active_inside_subagent`
- [x] 7.7 RED tool calls internas del subagente no piden aprobación adicional más allá de la de `delegate`. Test: `tests/subagents/test_delegate.py::test_internal_subagent_calls_do_not_reask_approval`
...
## Lote 9 — Verificación final y smoke E2E — [x] COMPLETO (4/4)

- [x] 9.1 Ejecutar `pytest -q`, `ruff check .`, `mypy src/erickfp`, `lint-imports`: 0 fallos, cobertura ≥85%. Resultado: 247 passed (243 + 4 nuevos del fix de 9.2), cobertura 95%, ruff/mypy/lint-imports limpios.
- [x] 9.2 Smoke E2E real (GEMINI_API_KEY vigente): `erickfp chat` — banner (+fallback), `/help /model /tools /clear /tokens`, un turno con tool call aprobado y uno denegado, `/tokens` tras el turno. **Hallazgo critico durante el smoke**: `stop_reason` crudo de litellm ("stop"/"tool_calls") cruzaba sin normalizar hacia `Response.stop_reason`, mientras `Agent.run_turn` compara contra la convencion canonica ("end_turn"/"tool_use") — ninguna tool se ejecutaba jamas contra el modelo real pese a que los 243 tests con dobles pasaban en verde. Corregido con TDD (RED: `tests/provider/test_litellm_gemini.py`, 2 tests actualizados + 1 nuevo parametrizado x4; GREEN: `_STOP_REASON_MAP` en `litellm_gemini.py::_to_response`). Re-verificado contra el modelo real tras el fix: tool call aprobada ejecuta de verdad (archivo marcador creado en disco), tool call denegada bloquea de verdad (archivo NUNCA creado).
- [x] 9.3 Documentado el smoke en `docs/smoke-e2e-v0-2.md` con evidencia real (comandos, transcripciones sanitizadas, verificacion directa en el SQLite real de 6 resúmenes de sesión persistidos, confirmación de arranque limpio sin `mcp.json`).
- [x] 9.4 Tabla de trazabilidad final (`docs/smoke-e2e-v0-2.md`, sección 8): 55/55 escenarios reales de las 11 specs (el conteo real de encabezados `Scenario:` es 55, no 56 — discrepancia de la estimación de `sdd-tasks` documentada explícitamente) mapeados a un test real que pasa hoy (247 tests totales).
